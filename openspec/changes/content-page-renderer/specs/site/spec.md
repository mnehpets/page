...
The system SHALL use a two-phase approach in the FS-backed implementation: metadata (front matter / JSON-LD) is parsed at `NewSite` construction time to build the index; the page body is re-read from the FS at render time and is always current.
